...
function assertAssessmentSubject(intent, assessment) {
  if (assessment.issuer.canonical_identifier !== intent.agent.canonical_identifier) {
    const error = new Error(
      "Eligibility assessment subject does not match listing intent canonical identifier"
    );
    error.code = "ELIGIBILITY_SUBJECT_MISMATCH";
    error.expected = intent.agent.canonical_identifier;
    error.received = assessment.issuer.canonical_identifier;
    throw error;
  }
}
...
function evaluateListingIntentBundle(intent, generatedAt = new Date(), options = {}) {
  ...
  assertAssessmentSubject(intent, assessment);
